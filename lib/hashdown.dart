...
class HashdownParams {
  ...
  HashdownParams.fromOption(HashdownOptions opt) {
    if ((opt.password != '' && opt.password != null) ||
  ...
  int toByte() {
    return 0xC0 | (compressed << 5) | (protection << 3) | (markdown <<
        2) | mode;
  }
}
...
class Hashdown {
  ...
  static const String PROTECT_AUTO = 'autosalt';
  ...
  static final RegExp shadowEncodeReg = new RegExp(
      r'(^|[^\\])\{[^\u0000]*?[^\\]\}');
  ...
  static String _encodeShadowCode(String str, HashdownOptions opt) {
    ...
    return str.replaceAllMapped(shadowEncodeReg, replaceShadowCode).replaceAll(
        '\\{', '{').replaceAll('\\}', '}');
  }
